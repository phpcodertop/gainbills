...
class ads_model extends CI_Model
{
	...
	public function get_ads($limit,$offeset,$t='result'){
		if($t == 'result'){
		...
		$result= $this->db
		->select('ads.id as ads_id,title,description,image,duration,points')
        ->from('ads')
		->order_by('order','asc')
        ->get();
       if($t == 'result'){
	...
	public function get_adv($id,$user_id){
        ...
        if(!empty($clicks))	{
		$query= $this->db
		             ->select('ads.id as adv_id,url,clickable,user_id,clicks_num,duration,points,title')
		             ->from('ads')
		 ...
		 // print_r($data);
           ...
           $this->db->update('user_history');
			...
			$query= $this->db
		                  ->select('id as adv_id,url,duration,points,title')
		                  ->from('ads')
                          ->where('id',$id)
		                  ->get()
		                  ->result();
		 ...
		 $query= $this->db
		              ->select('ads.id as adv_id,url,duration,points,title')
		              ->from('ads')
                      ->where('ads.id',$id)
		              ->get()
		              ->result();	
   ...
   public function add_coins($email,$ads_points){
	   //get account coins
	   $this->db
	        ->select('points')
			->where('email',$email)
			->from('users');
		$points_arr= $this->db->get()->result();
		foreach($points_arr as $points):
		$user_point=$points->points;
		endforeach;
		$new_points=$user_point+$ads_points;
